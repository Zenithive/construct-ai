...
const AuthForm: React.FC<AuthFormProps> = ({
  ...
  const handleEmailChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setEmail(v);
    setEmailError(validateEmail(v) ? "" : "Invalid email address");
  };

  const handlePasswordChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const v = e.target.value;
    setPassword(v);
    if (passwordError && validatePassword(v)) setPasswordError("");
  };

  const handleFirstNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const v = e.target.value;
    setFirstName?.(v);
    if (firstNameError && v.trim()) setFirstNameError("");
  };

  const handleLastNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const v = e.target.value;
    setLastName?.(v);
    if (lastNameError && v.trim()) setLastNameError("");
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateEmail(email)) { setEmailError("Invalid email format"); hasError = true; }
    else setEmailError("");

    if (!validatePassword(password)) {
      setPasswordError("Must have uppercase, lowercase, number & special character");
      hasError = true;
    ...
    if (showCaptcha) {
      ...
      if (!recaptchaToken) { setRecaptchaError("Please complete the reCAPTCHA"); hasError = true; }
  ...
  const inputClass = (hasError: boolean) =>
    ...
    } rounded-xl text-gray-900 placeholder-gray-500 text-sm focus:outline-none focus:ring-4 ${
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <div className="relative">
          ...
          <button
            ...
            className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 focus:outline-none p-1 rounded-lg hover:bg-gray-100 transition-colors"
      ...
      <button
        type="submit"
        disabled={isLoading}
        className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white py-3 px-4 rounded-xl font-semibold text-sm shadow-lg shadow-blue-500/30 hover:shadow-xl hover:shadow-blue-500/40 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none flex items-center justify-center mt-6"
      >
        {isLoading ? (
          <>
