...
const ChatComponent = ({ selectedRegion, selectedCategory, regions, categories, sessionId, messages, isLoading, streamingSources, onSetMessages, onRunStream, onMessageSent, onToggleSidebar, isSidebarOpen }: ChatComponentProps) => {
  ...
  const renderMessageContent = (content: string, sources?: Source[]) => <div className="text-base max-w-none leading-relaxed break-words" dangerouslySetInnerHTML={{ __html: renderContent(content, sources || []) }} />;
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div id="chat-scroll-area" ref={scrollContainerRef} onScroll={handleScroll} className="p-4 sm:p-6 space-y-6 bg-white relative" style={{ flex: '1 1 0', overflowY: 'scroll', overflowX: 'hidden', maxHeight: '100%', minHeight: 0 }}>
        {showHistory ? (
          <div className="text-center py-12"><div className="animate-spin rounded-full h-8 w-8 border-2 border-gray-300 border-t-blue-600 mx-auto mb-3" /><p className="text-gray-400 text-sm">Loading chat history...</p></div>
        ) : messages.length === 0 ? (
          <div className="text-center py-12 sm:py-20 max-w-xl mx-auto">
            <div className="inline-flex items-center justify-center w-14 h-14 bg-blue-50 border border-blue-100 rounded-2xl mb-5"><Search className="h-7 w-7 text-blue-600" /></div>
            <h3 className="text-2xl font-semibold text-gray-900 mb-2">How can I help you today?</h3>
            <p className="text-base text-gray-500 mb-8">Ask about construction regulations, safety standards, or compliance requirements</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-left">
              {sampleQuestions.map((question, index) => (
                <button key={index} onClick={() => setInputMessage(question)} className="p-4 bg-white border border-gray-200 hover:border-gray-300 hover:bg-gray-50 rounded-xl text-base text-gray-600 text-left transition-colors">{question}</button>
              ))}
            </div>
          </div>
        ) : (
          <>
            {messages.map((message, index) => (
              <div key={index}>
                {message.type === 'user' && (
                  <div className="flex justify-end"><div className="max-w-[80%] sm:max-w-2xl px-4 py-3 rounded-2xl bg-gray-100 text-gray-900"><div className="whitespace-pre-wrap text-base">{message.content}</div></div></div>
                )}
                {message.type === 'ai' && (() => {
                  ...
                  return (
                    <div className="flex justify-start mb-2">
                      <div className="bg-gray-50 border border-gray-200 rounded-xl p-3 max-w-2xl">
                        {isLastAndStreaming && <div className="flex items-center space-x-2 text-xs text-gray-500 mb-2"><div className="animate-spin rounded-full h-3 w-3 border-2 border-blue-500 border-t-transparent" /><span>Searching {webSources.length} sources…</span></div>}
                        <div className="flex gap-2 overflow-x-auto pb-1">
                          {webSources.map((source: any, idx: number) => {
                            let domain = ''; try { domain = source.url ? new URL(source.url).hostname.replace('www.', '') : ''; } catch {}
                            return <a key={`web-${idx}`} href={source.url || '#'} target="_blank" rel="noopener noreferrer" className="flex-shrink-0 w-36 p-2 border border-gray-200 rounded-lg hover:border-blue-300 hover:bg-white transition-all bg-white"><p className="text-xs font-medium text-gray-700 line-clamp-2 leading-tight">{source.title || 'Web Page'}</p><p className="text-xs text-gray-400 mt-1 truncate">{domain}</p></a>;
                          })}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="max-w-[80%] sm:max-w-2xl">
                      ...
                      {message.citations && (
                        <div className="mt-3 pt-3 border-t border-gray-100">
                          ...
                          {message.citations.map((citation, i) => <div key={i} className="text-xs mb-1.5 pl-3 border-l-2 border-blue-200 text-gray-500">{renderCitationContent(citation)}</div>)}
                          {message.confidence && <div className="mt-2 flex items-center space-x-2"><div className="flex-1 bg-gray-100 rounded-full h-1"><div className="bg-blue-500 h-1 rounded-full" style={{ width: `${message.confidence}%` }} /></div><span className="text-xs text-gray-400">{message.confidence}% confidence</span></div>}
                        </div>
                      )}
                    </div>
                  </div>
            ...
            {isLoading && messages.length > 0 && messages[messages.length - 1].type === 'ai' && streamingSources.db_sources.length === 0 && streamingSources.web_sources.length === 0 && (
              <div className="flex justify-start"><div className="flex items-center space-x-2 px-4 py-3"><div className="flex space-x-1"><div className="w-2 h-2 bg-gray-400 rounded-full animate-bounce" style={{ animationDelay: '0ms' }} /><div className="w-2 h-2 bg-gray-400 rounded-full animate-bounce" style={{ animationDelay: '150ms' }} /><div className="w-2 h-2 bg-gray-400 rounded-full animate-bounce" style={{ animationDelay: '300ms' }} /></div></div></div>
            )}
            <div ref={messagesEndRef} />
          </>
        )}
        {!isAutoScroll && messages.length > 0 && (
          <button onClick={() => { setIsAutoScroll(true); scrollToBottom(); }} className="fixed bottom-24 right-6 bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 p-2.5 rounded-full shadow-md transition-all z-10" aria-label="Scroll to bottom"><ArrowDown className="h-4 w-4" /></button>
        )}
      </div>
      ...
      <div className="flex-shrink-0 bg-white px-4 pb-4 pt-3">
        <div className="max-w-2xl mx-auto">
          <div className="flex items-end space-x-2 bg-white border border-gray-300 rounded-2xl px-4 py-3 shadow-sm focus-within:border-gray-400 transition-colors">
            <input type="text" value={inputMessage} onChange={e => setInputMessage(e.target.value)} onKeyPress={e => e.key === 'Enter' && !e.shiftKey && handleSendMessage()} placeholder="Message ConstructAI..." className="mb-1 flex-1 bg-transparent text-base text-gray-900 placeholder-gray-400 focus:outline-none resize-none" />
            <div className="flex items-center space-x-1 flex-shrink-0 gap-2">
              <button onClick={() => setShowUploadModal(true)} className="p-1.5 text-gray-400 hover:text-gray-600 bg-gray-100 rounded-lg transition-colors" title="Upload documents"><Paperclip className="h-5 w-5" /></button>
              <button onClick={handleSendMessage} disabled={!inputMessage.trim() || isLoading} className="p-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg disabled:opacity-40 disabled:cursor-not-allowed transition-colors ml-4"><Send className="h-5 w-5" /></button>
            </div>
          </div>
          <p className="text-center text-sm text-gray-400 mt-2">ConstructAI can make mistakes. Verify important information.</p>
        </div>
      </div>
      ...
      {showUploadModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto relative">
            <button onClick={() => setShowUploadModal(false)} className="absolute top-4 right-4 text-gray-500 hover:text-gray-700 z-10">
