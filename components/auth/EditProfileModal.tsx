...
interface Props { firstName: string; lastName: string; email: string; onClose: () => void; onSave: (firstName: string, lastName: string) => Promise<void>; }
...
const EditProfileModal: React.FC<Props> = ({ firstName, lastName, email, onClose, onSave }) => {
  ...
  const handleSave = async () => {
    ...
    } catch (e: unknown) { setError(e instanceof Error ? e.message : 'Failed to update profile.'); }
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4 overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-black/[0.09]">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-[#E1F5EE] flex items-center justify-center"><User className="w-4 h-4 text-[#1D9E75]" /></div>
            <div><h2 className="text-sm font-medium text-[#111]">Edit Profile</h2><p className="text-xs text-[#999] mt-0.5">{email}</p></div>
          </div>
        ...
        <div className="px-5 py-5 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="text" value={first} onChange={e => { setFirst(e.target.value); setError(''); }} placeholder="First name" className={inputClass(false)} />
            </div>
            <div>
              <label className="block text-xs text-[#555] mb-1.5 font-medium">Last name</label>
              <input type="text" value={last} onChange={e => { setLast(e.target.value); setError(''); }} placeholder="Last name" className={inputClass(false)} />
            </div>
          </div>
          <div>
            <label className="block text-xs text-[#555] mb-1.5 font-medium">Email</label>
            <input type="email" value={email} disabled className="w-full px-4 py-3 bg-[#f0f0ec] border border-black/[0.06] rounded-lg text-[#999] text-sm cursor-not-allowed" />
            <p className="text-[11px] text-[#999] mt-1 ml-0.5">Email cannot be changed.</p>
          </div>
          {error && <div className="flex items-center gap-2 px-3.5 py-2.5 bg-red-50 border border-red-200 rounded-lg"><AlertCircle className="w-4 h-4 text-red-500 flex-shrink-0" /><p className="text-xs text-red-700">{error}</p></div>}
          {success && <div className="flex items-center gap-2 px-3.5 py-2.5 bg-[#E1F5EE] border border-[#5DCAA5]/30 rounded-lg"><CheckCircle2 className="w-4 h-4 text-[#1D9E75] flex-shrink-0" /><p className="text-xs text-[#0F6E56]">Profile updated successfully.</p></div>}
        </div>
        <div className="flex items-center justify-end gap-2.5 px-5 py-4 border-t border-black/[0.09] bg-[#f7f7f5]">
          ...
          <button onClick={handleSave} disabled={isLoading || !hasChanges || success}
