        ...
        <div className="px-6 py-5 space-y-5">
            ...
            <div className="space-y-2">
              {PLAN_OPTIONS.map(opt => (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-4 py-3 rounded-lg border text-left transition-all duration-150 ${
                    planType === opt.value
                      ? 'border-[#1D9E75] bg-[#E1F5EE]'
                      : 'border-black/[0.09] bg-[#f7f7f5] hover:border-black/[0.18]'
                  }`}
                >
                  <div>
                    <span className={`text-sm font-medium ${planType === opt.value ? 'text-[#0F6E56]' : 'text-[#111]'}`}>
                      {opt.label}
                    </span>
                    <span className="block text-xs text-[#999] mt-0.5">{opt.desc}</span>
                  </div>
                  <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
                    planType === opt.value ? 'border-[#1D9E75]' : 'border-[#ccc]'
                  }`}>
                    {planType === opt.value && (
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-3 bg-[#f7f7f5] border border-black/[0.09] rounded-lg text-sm text-[#111] focus:outline-none focus:ring-2 focus:ring-[#E1F5EE] focus:border-[#1D9E75] transition-all duration-150"
            >
          ...
          {planType === 'free' && user.planType !== 'free' && (
            <div className="flex items-start gap-2.5 px-3.5 py-3 bg-amber-50 border border-amber-200 rounded-lg">
              ...
              <p className="text-xs text-amber-700">
                Downgrading to Free will reset the user's usage and remove their paid access immediately.
              </p>
            </div>
          )}
